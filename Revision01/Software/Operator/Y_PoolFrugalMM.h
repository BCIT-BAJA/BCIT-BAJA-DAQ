...
struct Y_PoolFrugalMM {
	...
	alignas(kCacheLineSize) std::atomic<uint64_t> m_head_a { 0 };
	...
	bool Create(const uint32_t arg_entries_capacity) {
		...
		m_head_a.store(0, std::memory_order_relaxed);
	...
	bool _M_RentRX_TryExclusive(const uint64_t index, Y_PoolRental<T>* out) {
		Task_ZoneScoped_NoCallstack;
		...
		if(exclusive % 2 == 0) {
			...
			*/
			if(entry->exclusive.compare_exchange_strong(exclusive, (exclusive + 1), std::memory_order_acquire)) {
				out->key = cast(void*)cast(uintptr_t)index;
				out->key_epoch = (exclusive + 1);
				Memcpy(&out->value, entry->storage.mCharData, sizeof(T));
				return true;
			}
		}
	...
	Y_Rx_e M_Rent(Y_PoolRental<T>* out) {
		...
		head_i = (head_i + m_entries_capacity - 1) % m_entries_capacity;
		if(_M_RentRX_TryExclusive(head_i, out)) { return Y_Rx_e::Success; }
		...
		head_i = m_head_a.fetch_add(1, std::memory_order_relaxed);
		head_i %= m_entries_capacity;
		if(_M_RentRX_TryExclusive(head_i, out)) {
			return Y_Rx_e::Success;
		}
